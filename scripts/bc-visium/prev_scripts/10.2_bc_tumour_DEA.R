...
# Read sSeurat object.
seuratobj.tumour <- readRDS(file = "./results/analysis/seuratobj_only_tumour.rds")
bc.recomputed <- readRDS(file = "./results/analysis/beyondcell_allspots_breastsignature_tumour.rds")

# Export TCs from beyondcell object (for both resolutions)
bc.recomputed@meta.data
metadata.tcs <- bc.recomputed@meta.data %>%
  select(TCs_res.0.2)
tcs <- sort(unique(metadata.tcs$TCs_res.0.2))


# Create new seurat object with TC metadata
seuratobj.tcs <- AddMetaData(seuratobj.tumour, metadata = metadata.tcs)
...
# Resolution 0.1
Idents(seuratobj.tcs) <- "TCs_res.0.2"
...
write.table(dea.gsea, file = "./data/dea_tables/dea.gsea.tumour.tsv", sep = "\t",
            col.names = TRUE, row.names = FALSE, quote = FALSE)
write.table(dea.ora.0.5, file = "./data/dea_tables/dea.ora_0.5.tumour.tsv", sep = "\t",
            col.names = TRUE, row.names = FALSE, quote = FALSE)
write.table(dea.ora.1, file = "./data/dea_tables/dea.ora_1.tumour.tsv", sep = "\t",
            col.names = TRUE, row.names = FALSE, quote = FALSE)

saveRDS(seuratobj.tcs, file = paste0("./results/analysis/seuratobj.therapeutic.clusters.tumour.rds"))
